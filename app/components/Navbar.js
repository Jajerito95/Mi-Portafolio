...
export default function Navbar() {
  ...
  const links = [
    { href: esHome ? "#inicio" : "/", label: "Inicio", id: "inicio" },
    { href: esHome ? "#sobre-mi" : "/#sobre-mi", label: "Sobre mí", id: "sobre-mi" },
    { href: esHome ? "#discord" : "/#discord", label: "Staff", id: "discord" },
    { href: esHome ? "#guias" : "/#guias", label: "Guías", id: "guias" },
    { href: esHome ? "#contacto" : "/#contacto", label: "Contacto", id: "contacto" },
  ]
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0a0a0f]/80 backdrop-blur-md border-b border-purple-900/30">
      <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">

        <a href="/" onClick={() => sessionStorage.setItem("visitaInterna", "true")} className="text-xl font-bold">
        ...
        <div className="hidden md:flex gap-6 text-sm font-medium">
          {links.map((link) => (
            <a key={link.id} href={link.href} onClick={() => { if (!esHome) sessionStorage.setItem("visitaInterna", "true") }} className={`transition-all duration-200 relative pb-1 ${esHome && seccionActiva === link.id ? "text-purple-400" : "text-gray-400 hover:text-purple-400"}`}>
              {link.label}
              {esHome && seccionActiva === link.id && (
      ...
      <div className={`md:hidden transition-all duration-300 overflow-hidden ${menuAbierto ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="px-4 pb-4 flex flex-col gap-2 border-t border-purple-900/30 pt-4">
          {links.map((link) => (
            <a key={link.id} href={link.href} onClick={() => { setMenuAbierto(false); if (!esHome) sessionStorage.setItem("visitaInterna", "true") }} className={`transition-colors duration-200 py-2 ${esHome && seccionActiva === link.id ? "text-purple-400 font-semibold" : "text-gray-400 hover:text-purple-400"}`}>
              {link.label}
            </a>
          ))}
        </div>
